...
public class CreateProcessPanel {
    ...
    if (totalHeight > scrollPaneHeight) {
    	...
    	processedMenuPanel.setPreferredSize(new Dimension(0, totalHeight));
    } else {
        // 스크롤이 필요하지 않은 경우에는 스크롤 패널의 크기를 기존 설정값으로 유지
    	processedMenuPanel.setPreferredSize(new Dimension(0, scrollPaneHeight));
    ...
    JScrollPane scrollPane = (JScrollPane) processedMenuPanel.getParent().getParent(); // processedMenuPanel의 부모 컨테이너인 JScrollPane 가져오기
